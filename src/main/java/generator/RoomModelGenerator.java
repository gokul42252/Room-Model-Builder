...
public class RoomModelGenerator {
    private static String PATH;
    public static void generate(String canonicalPath, String jsonString,
                                String className, boolean isAnnotationRequired,
                                boolean isNeedConstructor,
                                boolean isGetterSetterRequired) {
        RoomModelGenerator.PATH = canonicalPath;
        try {
            ...
            extractVariablesFromJsonObject(jsonObject, className, variableBuilder,
                    importsBuilder, getterSetterBuilder, constructorBuilder,
                    constructorAssignmentBuilder, isAnnotationRequired,
                    isNeedConstructor, isGetterSetterRequired);
        } catch (Exception e) {
    ...
    private static void extractVariablesFromJsonObject(
            JSONObject jsonObject,
            String name,
            StringBuilder variableBuilder,
            StringBuilder importsBuilder,
            StringBuilder getterSetterBuilder,
            StringBuilder constructorBuilder,
            StringBuilder constructorAssignmentBuilder,
            boolean isAnnotationRequired,
            boolean isNeedConstructor,
            boolean isGetterSetterRequired) {
        if (jsonObject == null) {
            return;
        ...
        importsBuilder.append("import android.arch.persistence.room.ColumnInfo;\n" +
                "import android.arch.persistence.room.Entity;").append("\n");
        importsBuilder.append("import android.arch.persistence.room.Ignore;\n" +
                "import android.arch.persistence.room.PrimaryKey;").append("\n");
        ...
        variableBuilder.append("  @PrimaryKey(autoGenerate = true)").append("\n");
        variableBuilder.append("  @ColumnInfo(").append("name=").append("\"").append("_id").append("\"").append(")").append("\n");
        variableBuilder.append("  private Integer ").append("_id").append(";");
        variableBuilder.append("\n");
        while (keyIterator.hasNext()) {
            String field = keyIterator.next();
            if (isAnnotationRequired) {
                variableBuilder.append("  @ColumnInfo(").append("name=").append("\"").append(field).append("\"").append(")").append("\n");
            }
            try {
    ...
    private static void createFileStucture(String name, BufferedWriter bufferedWriter, StringBuilder importsBuilder) {
        try {
            ...
            bufferedWriter.write("/**\n" +
                    " * Room Table Builder\n" +
                    " * */\n");
            bufferedWriter.write("@Entity(tableName = \"" + name + "\")");
            bufferedWriter.write("\n");
